...
// Database Helper (Dilengkapi pengaturan link folder MEGA)
const readDB = () => {
...
// Helper: Cari atau Buat Folder secara aman di MEGA (Diperbaiki agar tidak crash)
async function getFolder(parent, name) {
    try {
        ...
        if (!folder) {
            ...
            // Mendaftarkan folder baru ke memori agar langsung ditemukan tanpa perlu reload API
...
// Helper: Mencari folder kustom milik Admin di dalam akun MEGA yang sedang login
...
// API Simpan & Ambil Pengaturan Link Folder MEGA
...
app.post('/api/categories', adminOnly, (req, res) => {
    const db = readDB();
    const newCat = { id: 'cat_' + Date.now(), albumId: req.body.albumId, name: req.body.name };
...
app.put('/api/photos/:id', adminOnly, (req, res) => {
    ...
    if (item) { item.caption = req.body.caption; writeDB(db); } res.json({ success: true });
});
...
app.post('/api/photos', adminOnly, upload.array('photos', 50), async (req, res) => {
    try {
        ...
        // 1. Tentukan Folder Target yang terautentikasi dengan akun Anda
        ...
        if (db.settings.megaFolderUrl) {
            try {
                ...
                console.log("Folder custom tidak ditemukan di pohon akun, menggunakan root folder.");
            }
        }

        // Pastikan struktur folder dibuat secara rapi: BaseFolder > MoodboardApps > [Nama Album]
        ...
        let counter = db.photos.filter(p => p.categoryId === categoryId).length + 1;

        // 2. Looping Upload dengan Penamaan Baru & Auto-Fallback jika terjadi error jaringan
        for (const file of req.files) {
            const ext = path.extname(file.originalname) || '.jpg';
            // FORMAT PENAMAAN: [Nama Album] - [Jenis Kategori] - [Nomor].[ext]
            const newFileName = `${targetAlbum.name} - ${targetCategory.name} - ${counter}${ext}`;

            let megaLink = "";
            try {
                // Upload dengan opsi ukuran eksplisit agar stabil di megajs
                ...
                console.log("Upload ke subfolder mengalami kendala, mengalihkan ke root folder MEGA...", uploadErr.message);
...
app.delete('/api/photos/:id', adminOnly, async (req, res) => {
    ...
    // Coba hapus fisik di MEGA, jika gagal TIDAK APA-APA, lanjut hapus di DB
